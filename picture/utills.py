    ...
    def plot_single(self, keys: List[str]):
        ...
        for key in keys:
            ...
            for column in results.keys():
                if column == "epoch":
                    continue
                plotResultsWithErrors(results[column], "epoch", column, os.path.join(self.save_path, f"{key}_{column}.png"))

    def plot_compare(self, keys: List[str], column: str):
        """
        Plot the comparison figure.
        """
        results = {}
        for key in keys:
            results[key] = self.data[key][column]
        plotMutiResultsWithErrors(results, "epoch", column, os.path.join(self.save_path, f"{keys}_{column}__compare.png"))

    def plot_compare_all(self, keys: List[str]):
        """
        Plot the comparison figure for all columns.
        """
        for column in self.data[keys[0]].keys():
            results = {}
            for key in keys:
                results[key] = self.data[key][column]
            plotMutiResultsWithErrors(results, "epoch", column, os.path.join(self.save_path, f"{keys}_{column}__compare.png"))
